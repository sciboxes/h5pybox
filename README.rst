...
the virtual machine, and invoking ``vagrant reload --provision``. Furthermore,
it is more verbose. You get to see a report from salt, which is not shown when
using vagrant. (*There's perhaps a way to see it though.*)

The salt report is especially useful when `troubleshooting`_ the provisioning
phase, and trying different configurations to fix the issue.
